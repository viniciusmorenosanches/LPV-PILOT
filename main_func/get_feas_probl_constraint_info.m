...
if ~isempty(mpc.x_min) || ~isempty(mpc.x_max)
    [fi_s_min_x0,fi_s_max_x0] = fi_box_fun(s,mpc.x_min,mpc.x_max,mpc.Nx,mpc.nx,v);
...
if feas && (~isempty(mpc.x_ter_min) || ~isempty(mpc.x_ter_max))
    [fi_s_ter_min_x0,fi_s_ter_max_x0] = fi_box_fun(s_ter,mpc.x_ter_min,mpc.x_ter_max,mpc.nx,mpc.nx,v);
...
if feas && (~isempty(mpc.u_min) || ~isempty(mpc.u_max))
    [fi_u_min_x0,fi_u_max_x0] = fi_box_fun(u,mpc.u_min,mpc.u_max,mpc.Nu,mpc.nu,v);
...
if feas && (~isempty(mpc.du_min) || ~isempty(mpc.du_max))
    [fi_du_min_x0,fi_du_max_x0] = fi_box_fun(du,mpc.du_min,mpc.du_max,mpc.Nu,mpc.nu,v);
...
if feas && (~isempty(mpc.y_min) || ~isempty(mpc.y_max))
    [fi_y_min_x0,fi_y_max_x0] = fi_box_fun(y,mpc.y_min,mpc.y_max,mpc.Ny,mpc.ny,v);
...
if feas && mpc.ter_ingredients && mpc.ter_constraint
    fi_ter_x0 = get_terConst_val(x_ref,s_ter,mpc.P,v);
    if fi_ter_x0>=0 
        feas = 0;
    end
else
